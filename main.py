...
def check_image_click(pos):
    ...
    for idx, (image, img_pos) in enumerate(zip(images, image_positions)):
        ...
        if rect.collidepoint(pos):
            # The variable below is used to hold the value of the selected action
            clicked_image_index = idx + 1
    ...
    while running:
        ...
        if not start:
            ...
            if func == 4:
                ...
                if times > 0:
                    call_text_lines(
                        "Find the exact lines the word was detected inside lines.txt(directory of the program)")
            if func == 5:
                if flag_tables == True:
